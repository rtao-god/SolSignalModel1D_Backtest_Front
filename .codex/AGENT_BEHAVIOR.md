...
- Для frontend порог добавления комментариев ниже, чем раньше: комментарии нужны не только в сложных, но и в средних по сложности местах, если они помогают быстро понять файл без знания контекста проекта.
- В каждом нетривиальном `.ts/.tsx` файле должен быть хотя бы один полезный комментарий, который сокращает время входа новому разработчику.
...
- Для компонентов и модулей средней сложности тоже добавляй главный комментарий, если без него новому разработчику трудно быстро понять роль файла, входные контракты, источники данных или ограничения поведения.
...
- Если файл не тривиален, отсутствие хотя бы одного полезного комментария считать дефектом сопровождения.
...
- Детальные правила структуры, полноты и оформления tooltip задаются в `SolSignalModel1D_Backtest_Front/AGENTS_FRONTEND.md`.
- В этом файле не задавай отдельный конкурирующий стиль для `description`; здесь остаётся только правило, когда `TermTooltip` вообще нужен.
