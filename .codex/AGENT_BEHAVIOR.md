...
В каждом файле (НЕ воспринимать буквально слово "в каждом". Кроме, понятное дело, scss файлов и прочих файлов, которые под это не рассчитаны и им это тупо не нужно) обязательно писать русскоязычные комментарии (англ. термины можно).
...
/*
	ModelStatsPage — UI для просмотра отчёта “Статистика моделей”, которая в себя.
...
- В файлах .scss тоже нужно обязательно писать комментарии, но не в формате выше с заголовками и разделами и прочим форматом, а просто нужны обычные комментарии для понимания стилей, ничего более. Тем более писать главный комментарий нельзя в scss файлах.  
...
interface PredictionHistoryPageProps {
    className?: string
}" и "SectionPager — UI модуль.

	Зачем:
		- Инкапсулирует UI-компонент или вспомогательные типы." НЕ надо, потому что это МУСОРНЫЕ комментарии и тупые по своей сути. Вместо 		- Инкапсулирует UI-компонент или вспомогательные типы." НЕ надо, потому что это МУСОРНЫЕ комментарии и тупые по своей сути. Вместо них обязаны быть нормальные, подрорбные, без мусора и лишней информации, комментарии, главный комментарий и обычный (и да, это касается лишь для .tsx файлов и некоторых .ts файлов (там, где интерфейс, обычно не нужен вообще главный комментарий. Там, где всякие константы и прочее - нужен, но без бреда и мусорной информации, которая и там понятна и видна с первого взгялда. В комментариях не должно быть каких-то лишних слов ради слов) 
...
- Формат обычного компонента такой: отдельный module.scss файл, отдельный types.ts, который находится или рядом с компонентом самим (если он лежит не в ui папке), или выше на 1 уровень ../ рядом с index.ts (реимпорты) (если они есть, конечно)

- Пример структуры папок (обычные компоненты, для каких-то особенных/других компонентов можно спокойно отойти от структуры): pages/predictions/ui/CurrentMLModelPredictionPage/ui/CurrentMLModelPredictionPage.tsx. Ts файл с интерфейсом находится в pages/predictions/ui/CurrentMLModelPredictionPage/types.ts. Пример содержимого файла: export default interface CurrentMLModelPredictionProps {
    className?: string
}
...
- Так же стараться добавлять index.ts файл с реимпортом. Пример пути: 
- Пример самого файла: export { default as CurrentMLModelPredictionPage } from './ui/CurrentMLModelPredictionPage'. - Комментарии в подобных файлах писать НЕ нужно, потому что и так понятно, что тут происходит. Он нужен для того, чтобы сократить путь в импортируемом файле.
