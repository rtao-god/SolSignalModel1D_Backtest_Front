 ...
 */
export function readDocsTermItems(
    i18n: I18nInstance,
    key: string,
 ...
 */
export function readDocsStringList(i18n: I18nInstance, key: string, namespace: DocsI18nNamespace = 'docs'): string[] {
    const resourceName = `${namespace}.i18n`
    const value = readActiveLocaleResource(i18n, namespace, key, resourceName)
    if (!Array.isArray(value)) {
 ...
 */
export function readDocsTableRows(i18n: I18nInstance, key: string, namespace: DocsI18nNamespace = 'docs'): string[][] {
    const resourceName = `${namespace}.i18n`
    const value = readActiveLocaleResource(i18n, namespace, key, resourceName)
    if (!Array.isArray(value)) {
...
export function readAvailableDocsTermGroups(
    ...
    return keys.flatMap(key => {
        try {
            return [readDocsTermItems(i18n, key, namespace)]
        } catch {
            return []
        }
    })
